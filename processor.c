...
void processor(char* input, int *historySize, char* myhistory[20], int *nextHistory){
	...
	while(line[i] != NULL){
		...
		//new "cd" command with optional argument
		if(strcmp(cmd[0], "cd") == 0)
		{
			if(!cmd[1]) //if user does not enter an argument with the command
			{
			...
			if(cmd[1]) //if user enters an argument with command
			{
				chdir(cmd[1]);
				system("pwd");
			}	
		...
		if(pid == 0)		// CHILD
		{
			...
			//*********************************************************************************************************************//
			/*Checking for a '<' & '>' symbol and storing strings with commands/file names seperately.*/
			int redirect = 0; //bool statement
			int piper = 0;	  //bool statement

			for(int p=0; p < j; p++) //loop to run through everything within cmd[]
			{
				if(strchr(cmd[p], '<')!=NULL) //checking for an instance of a redirection symbol within cmd[]
				{
					int ifp; //input file
					redirect = 1; 
					char inputF[256]; //user specified file
					char *redirectionCMD [j]; //char array for all commands user enters
					
					for(int k=0; k <= j; k++)
					{
						char *strResult = strchr(cmd[k], '<'); // returns a value once symbol is found

						if(strResult == NULL) //searching for redirectino signal
						{
							redirectionCMD[k]=cmd[k]; //everything on the left side of the symbol will be plugged into an array.
						}
						if(strResult != NULL) //found redirection signal.
						{
							redirectionCMD[k]='\0'; 
							strcpy(inputF, cmd[++k]);       //user specified input file is 1 space after redirection signal.
							redirectionCMD[k]='\0';
							break;
					...
					ifp = open(inputF, O_RDONLY);	//opening input file
					dup2(ifp,fileno(stdin)); //changing FD
					close(ifp); //closing
					execvp(redirectionCMD[0], redirectionCMD); //executing the commands 
					dup2(0,fileno(stdin)); //revert back to normal
					printf("%s: command not found\n", redirectionCMD[0]);
					break;
				}
				/*all code the same as above only difference is searching for the '>' symbol.*/
			 	if(strchr(cmd[p], '>')!=NULL)	//'>' checking for apperance.
                                {
                                        redirect = 1;
                                        int ofp;
					char inputF[256];
					...
					/*opening output file, changing FDs, closing, and executing commands, then revert back to normal FD.*/
			...
			/*checking that the bool is valid in order to break.*/
			if(redirect==1)
			{
				//printf("%d\n", redirect);
				redirect = 0;
